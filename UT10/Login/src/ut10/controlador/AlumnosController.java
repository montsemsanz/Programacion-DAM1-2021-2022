    ...
    void listar(ActionEvent event) {
        ...
        try (Connection conexion = UtilsConexion.getConexion(BD);
            ...
            try (ResultSet rs = pst.executeQuery()) {
                while (rs.next()) {
                    String strFormateada = String.format("%-20s%-20s%-10d", rs.getString(2), rs.getString(1),
                            rs.getInt(3));
        ...
        try (Connection conexion = UtilsConexion.getConexion(BD);
            ...
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.isBeforeFirst()) {
                    ...
                    String strCabecera = String.format("%-20s%-20s%-10s%10s", rsm.getColumnName(3).toUpperCase(),
                    ...
                    while (rs.next()) {
                        String strFormateada = String.format("%-20s%-20s%-10d%10s", rs.getString(3), rs.getString(2),
